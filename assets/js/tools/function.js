...
const countDown = (datime=0, target, msg) => {
  let countDownDate = new Date(datime).getTime();
  let x = setInterval(function() {
    ...
    $(target).text("Timeout : " + hours + "h " + minutes + "m " + seconds + "s ");
...
const pagingRole = () => {
  return $('meta[name="paging-menu"]').attr('content');
}
...
const menu = (path="") => {
  return $('meta[property="og:menu"]').attr('content') + path;
}

const subMenu = (path="") => {
  return $('meta[property="og:sub_menu"]').attr('content') + path;
}

const postField = (url, type='GET', params=false, headers=false, opt=false, progressBar=false, index=false) => {
      ...
      xhr: function() {
        ...
        xhr.upload.addEventListener("progress", function(evt) {
          if (evt.lengthComputable) {
            ...
            if (progressBar) {
              progressBar(percentComplete, index, true);
            }
          }
        }, false);
...
const multiShowPasswd = (input1, input2, attr2) => {
  let input = $("#"+input1);
  let inputConfrm = $("#"+input2);
  let btn = $("#"+attr2);

  if (input.attr('type') == "password") {
    btn.html('<i class="fas fa-fw fa-eye"></i>');
    input.attr('type', 'text');
    inputConfrm.attr('type', 'text');
  } else {
    btn.html('<i class="fas fa-fw fa-eye-slash"></i>');
    input.attr('type', 'password');
    inputConfrm.attr('type', 'password');
  }
}
...
const filterLengthCKEDITOR = (data, min=false, max=false) => {
  const txt = data.trim();
  if (min && txt.length < min ) return `Minimum length ${min} characters`;
  if (max && txt.length > max ) return `Maximum length ${max} characters`;
  return true;
...
const filterNumb = (ev, status=false, min=false, max=false, must=false) => {
  ...
  for (let i = 0; i < array.length; i++) {
    if (array[i] != " " && array[i] >= 0 && array[i] <= 9 || array[i] == "." || array[i] == "," || array[i] == "-") {
      if (status == true) {
...
const getTimestamp = () => {
    ...
    const m = String(now.getMonth() + 1).padStart(2, '0'); // Months are 0-11
...
const renderWaferMap = (container, mapData, opt) => {
  const lines = mapData.trim().split('\n');
  const rows = lines.length;
  const cols = lines[0].length;

  container.css('grid-template-columns', `repeat(${cols}, 1fr)`);

  const htmlBuffer = [];
  $.each(lines, function(r, rowText) {
    const chars = rowText.split('');

    $.each(chars, function(c, char) {
      let statusClass = 'die-null';
      const key = Object.keys(opt).find(key => key === char);

      if (key) statusClass = opt[key]

      htmlBuffer.push(`<div class="die ${statusClass}" title="Row: ${r}, Col: ${c}"></div>`);
    });
  });

  container.html(htmlBuffer.join(''));
}
...
const loadingBackdrop = () => {
  ...
  <div class="modal-dialog modal-sm absolute-center" role="document">
...
const loadingBackdropSecond = () => {
  return `
  <div class="modal fade show d-block" id="finish-success" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="bg-transition backdrop"></div>
  <div class="modal-dialog modal-sm absolute-y-mid" role="document">
  <div class="modal-content bg-transparent border-0">
  <div class="modal-body p-0">
  <div class="mesh-loader">
  <div class="set-one">
...
async function getFileListFromZip(file) {
  let result = {};
