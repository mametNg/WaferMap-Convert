...
class Controller {
  ...
  public function view($view, $data = [])
  {
    ...
    $data['error-file'] = "";
    if (!file_exists($open)) $data['error-file'] = $view;
    if (!file_exists($open)) $open = 'app/views/errors/404.php';
    require_once dirname(__DIR__) . '/'. $open;
  }
  ...
  public function views($views=[], $data = [])
  {
    ...
    if (!$status) require_once dirname(__DIR__) . '/'. $open;
    if (!$status) exit();
  ...
  public function librarie($libraries)
  {
    require_once dirname(__DIR__) . '/app/libraries/'.$libraries.'.php';
    return new $libraries;
  }

  public function authApi($authApi)
  {
    require_once dirname(__DIR__) . '/app/api/'.$authApi.'.php';
    return new $authApi;
  }

  public function base_url($path='')
  {
    $base = (strlen($this->e($path)) == 0 ? $this->e(BASE_URL, "/") : $this->e(BASE_URL, "/") ."/". $this->e($path, "/"));
  ...
  public function base_port()
  {
    return SERVER_PORT;
  }

  public function sock_client()
  {
    return SOCK_CLIENT;
  }
  ...
  public function e($string="", $regex=" ")
  {
    return htmlspecialchars(addslashes(trim($string, $regex)));
  }

  public function non_ascii($string="")
  {
    // remove text non ascii
  ...
  public function is_unique($array=[])
  {
    return array_filter(array_unique($array));
  }

  public function multi_array_search_by_value($find, $array) {
    ...
    for ($i=0; $i < count($array); $i++) { 
      foreach ($array[$i] as $key => $val) if (strpos(" ".strtolower($val)." ", strtolower($find)) || strtolower($val) === strtolower($find)) return ["key" => $key, "val" => $val];
    }

    return false;
  }

  public function array_search_by_value($find, $array) {
    foreach ($array as $key => $val) if (strpos(" ".strtolower($val)." ", strtolower($find)) || strtolower($val) === strtolower($find)) return ["key" => $key, "val" => $val];

    return false;
  }
  
  public function array_sort($array, $find, $order=SORT_ASC)
  {
    // $order = SORT_DESC;
    $created_timestamps = array_column($array, $find);
    array_multisort($created_timestamps, $order, $array);

    return $array;
  }
 
  public function array_group($array, $key1=false, $key2=false, $key3=false) {
    $return = [];
    foreach($array as $val) {
      $key = ($key2 !== false && $key3 !== false ? $this->e($val[$key1]."-".$val[$key2]."-".$val[$key3], "-") : ($key2 !== false ? $this->e($val[$key1]."-".$val[$key2], "-") : $this->e($val[$key1], "-"))) ;
      $return[$key][] = $val;
    }
    return $return;
  }

  public function is_date($date=false, $format='Y-m-d')
  {
  ...
  public function clean($string, $regex="-") {
    $string = str_replace(' ', '-', $string);

    return preg_replace('/[^A-Za-z0-9\-]/', $regex, $string);
  }
  ...
  public function randNumb($length=12)
  {
    $txt = "1234567890";
    $string = "";
    for ($i=0; $i < $length; $i++) { 
  ...
  public function randChar($length=12)
  {
    $chars = "abcdefghijklmnopqrstuvwxyz";
    $txt = strtolower($chars).strtoupper($chars);
    $string = "";
    for ($i=0; $i < $length; $i++) { 
      $acak = rand(0, strlen($txt)-1);
      $string .= $txt[$acak];
  ...
  public function randString($length=12, $smb=false)
  {
    $chars = "abcdefghijklmnopqrstuvwxyz";
    $symbol = "!@#$%^&*()_-+=|[]{};:<>,.?";
    $txt = "1234567890".strtolower($chars).strtoupper($chars).($smb ? $symbol:"");
    $string = "";
    for ($i=0; $i < $length; $i++) { 
      $acak = rand(0, strlen($txt)-1);
      $string .= $txt[$acak];
  ...
  public function filterImg($myFile=[])
  {
    ...
    if (!in_array(strtolower(end($img['extension'])), $img['allowExtension'])) {
      return [
        'status'  => false,
    ...
    if ($img['pixel'] == false && strtolower(end($img['extension'])) !== 'svg') {
      return [
        'status'  => false,
  ...
  public function RSAdecrypt($field){
    ...
    if (!$privateKey = openssl_pkey_get_private($this->RSAPrivateKey())) return false;
      
    $decrypted_text = "";
    if (!openssl_private_decrypt(base64_decode($field), $decrypted_text, $privateKey)) return false;
    ...
    return (($time - $strTime) < 30 ? $decrypted_text : false);
  }
